...
def make_prompt(prompt_template: str, ex: dict) -> str:
    ...
    # Support both styles:
    # 1) template contains {question}
    # 2) template is just a prefix/instruction block
...
def _extract_candidate_expressions(answer_text: str) -> list[str]:
    ...
    for raw_line in answer_text.splitlines():
        ...
        # If there is an "=", use the LHS first, else use the whole line
        if "=" in line:
    ...
    # Fallback: entire answer block flattened
...
def countdown_reward_fn(response: str, ground_truth: str) -> dict[str, float]:
    ...
    # Only score answer correctness if there exists an expression that:
    # 1) evaluates to target
    # 2) uses exactly the allowed numbers (with multiplicity)
...
def grpo_train_loop(args):
    ...
    for grpo_step in range(args.n_grpo_steps):
        ...
        # Needed for:
        # - grpo_clip
        # - or multi-epoch reuse of same rollout data
        # --------------------------------------------------------------
        need_old_log_probs = (
        ...
        # --------------------------------------------------------------
        # 7. Aggregate richer logging metrics
        # --------------------------------------------------------------
        ...
        # --------------------------------------------------------------
        # 9. Periodic evaluation + checkpoint
        # --------------------------------------------------------------
