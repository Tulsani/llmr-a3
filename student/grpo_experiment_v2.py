...
def make_prompt(prompt_template: str, ex: dict) -> str:
    """Fill {question} placeholder with the actual countdown problem."""
    numbers = ex["numbers"]
    target  = ex["target"]
    problem = (
...
def evaluate_countdown(llm: LLM, examples: list[dict],
    ...
    # Append </answer> so truncated responses still get scored
    texts = [out.outputs[0].text + "</answer>" for out in outputs]

    print("DEBUG sample output:", repr(texts[0][:400]))
...
def grpo_train_loop(args):
    ...
    for grpo_step in range(n_grpo_steps):
        ...
        # 1. Sample questions and expand to rollout_batch_size upfront
        #    (one prompt per rollout, matching reference implementation)
        ...
        # 2. Generate one rollout per (expanded) prompt
        load_policy_into_vllm(policy, llm)
        ...
        # Append </answer> so truncated responses are still parseable
